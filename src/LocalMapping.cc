        ...
        for (size_t i = 0; i < vpNeighKFs.size(); i++)
        {
            ...
            for (int ikp = 0; ikp < nmatches; ikp++)
            {
                ...
                if (mpCurrentKeyFrame->vRgb.size())
                    pMP = new MapPoint(x3D_, mpCurrentKeyFrame, mpAtlas->GetCurrentMap(), mpCurrentKeyFrame->vRgb[idx1]);
                else
                    pMP = new MapPoint(x3D_, mpCurrentKeyFrame, mpAtlas->GetCurrentMap());
