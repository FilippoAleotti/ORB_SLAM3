...
namespace ORB_SLAM3
{
            ...
            if (!mpCamera2)
            {
                for (int i = 0; i < mCurrentFrame.N; i++)
                {
                    ...
                    if (z > 0)
                    {
                        ...
                        MapPoint *pNewMP;
                        if (pKFini->vRgb.size())
                            pNewMP = new MapPoint(x3D, pKFini, mpAtlas->GetCurrentMap(), pKFini->vRgb[i]);
                        else
    ...
    void Tracking::CreateInitialMapMonocular()
    {
        ...
        for (size_t i = 0; i < mvIniMatches.size(); i++)
        {
            ...
            MapPoint *pMP;
            if (pKFcur->vRgb.size())
                pMP = new MapPoint(worldPos, pKFcur, mpAtlas->GetCurrentMap(), pKFcur->vRgb[i]);
            else
                pMP = new MapPoint(worldPos, pKFcur, mpAtlas->GetCurrentMap());
    ...
    void Tracking::UpdateLastFrame()
    {
        ...
        for (size_t j = 0; j < vDepthIdx.size(); j++)
        {
            ...
            if (bCreateNew)
            {
                ...
                MapPoint *pNewMP;
                if (pRef->vRgb.size())
                    pNewMP = new MapPoint(x3D, mpAtlas->GetCurrentMap(), &mLastFrame, i, pRef->vRgb[j]);
                else
            ...
            if (!vDepthIdx.empty())
            {
                ...
                for (size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if (bCreateNew)
                    {
                        ...
                        MapPoint *pNewMP;
                        if (pKF->vRgb.size())
                            pNewMP = new MapPoint(x3D, pKF, mpAtlas->GetCurrentMap(), pKF->vRgb[i]);
                        else
                            pNewMP = new MapPoint(x3D, pKF, mpAtlas->GetCurrentMap());
    ...
    void Tracking::CreateNewMapPoints()
    {
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            for (int ikp = 0; ikp < nmatches; ikp++)
            {
                ...
                MapPoint *pMP;
                if (mpLastKeyFrame->vRgb.size())
                    pMP = new MapPoint(x3D, mpLastKeyFrame, mpAtlas->GetCurrentMap(), mpLastKeyFrame->vRgb[i]);
                else
